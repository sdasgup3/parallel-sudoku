...
_vertices = 1000 
_edgeDensity = 6 # _edges/_vertices [reference: https://hal.inria.fr/file/index/docid/91354/filename/BELIV_2006_Melancon_Final.pdf]
...
class subgraphGen:
  ...
  def generate(self, incoming):
    ...
    # populate edges
    edgeCount = 0
    while edgeCount < _edges/3:
      vertex1 = random.randint(0,_vertices/3-1)   # generate random number. Ends inclusive
      vertex2 = random.randint(0,_vertices/3-1)  
      if vertex1 == vertex2:
        continue
      if g.addEdge((vertex1, vertex2)):
        edgeCount = edgeCount + 1 

    while edgeCount < _edges/3*2:
      vertex1 = random.randint(_vertices/3, _vertices/3*2-1)   # generate random number. Ends inclusive
      vertex2 = random.randint(_vertices/3, _vertices/3*2-1)  
      if vertex1 == vertex2:
        continue
      if g.addEdge((vertex1, vertex2)):
        edgeCount = edgeCount + 1 
    ...
    while edgeCount < _edges:
      vertex1 = random.randint(_vertices/3*2, _vertices-1)   # generate random number. Ends inclusive
      vertex2 = random.randint(_vertices/3*2, _vertices-1)  
      if vertex1 == vertex2:
        continue
      if g.addEdge((vertex1, vertex2)):
        edgeCount = edgeCount + 1 
